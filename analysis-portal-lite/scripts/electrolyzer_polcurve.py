...
def run(input_dir: str, output_dir: str, params: dict = None) -> dict:
    ...
    for f in files:
        # Per-file output subdirectory if multiple files
        if len(files) > 1:
            file_out = str(out / f.stem)
        else:
            file_out = str(out)

        try:
            analyze(str(f), geo_area=geo_area, save_dir=file_out,
                    title=f.stem, T_C=T_C,
    ...
    # If ALL files failed, raise so the job shows as failed with the error
    if not successes and errors:
        raise RuntimeError(
            f"All {len(files)} file(s) failed:\n\n" + "\n---\n".join(errors)
        )

    output_files = [f.name for f in out.rglob('*') if f.is_file()]
    result = {
