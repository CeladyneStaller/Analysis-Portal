...
def _finite(v):
    """True for a real, JSON-representable number.

    NaN and infinity are floats and pass an isinstance check, but JSON cannot
    encode them: FastAPI renders with allow_nan=False, so one of them anywhere
    in a job record turns /api/jobs/<id> into a 500.
    """
